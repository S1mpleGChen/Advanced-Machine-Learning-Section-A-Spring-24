...
#decide whether to add outliers into original data
Outlier = False
outlier = np.random.randn(xvalue.shape[0], xvalue.shape[1])

#generate 2-d gaussian distribution, with each the mean as 0,1,2,3,
#the variance is set to be e-1 uniformly

Gaussian = True
g1 = np.random.normal(loc=0,scale=1e-1,size=(40,2)) 
g2 = np.random.normal(loc=1,scale=1e-1,size=(40,2))
g3 = np.random.normal(loc=2,scale=1e-1,size=(40,2))  
g4 = np.random.normal(loc=3,scale=1e-1,size=(40,2)) 

gaussian_mixture = np.vstack((g1,g2,g3,g4))
