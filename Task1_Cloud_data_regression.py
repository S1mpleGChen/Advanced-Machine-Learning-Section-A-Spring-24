...
def main_func(data_db, expert_fix, alpha):
    ...
    X_train = data_new[:-24, :-1]
    Y_train = data_new[:-24, -1:]
    X_test = data_new[-24:, :-1]
    Y_test = data_new[-24:, -1:]
...
if __name__ == "__main__":
    ...
    alpha = 0.8  #[0,1], can be adjusted for different settings 
    pit_series, loss_learner, loss_expert, pred_experts, pred_learner,Y_test = main_func(data_db1,expert_fix,alpha)
    ...
    evolution_obs(pit_series,'Weights evolution')
    #plot evolution of learner's cumulative loss
    learner_obs(cumuloss_learner, 'Learner loss evolution')
    evolution_obs(cumuloss_expert, 'Experts loss evolution')
    
    #plot evolution of experts' cumulative loss
    evolution_obs(pred_experts,'Prediction of 6 experts')
    learner_obs(pred_learner, 'Prediction of learner')
    learner_obs(Y_test, 'Label value')      
